...
namespace Application.DM_SuKienServices
{
    public class Update
    {
        ...
        {
            public DM_SuKienModalUpdate Entity;
        }
                ...
                using (SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            var result = await connection.QueryFirstOrDefaultAsync<DM_SuKien>("spu_DM_SuKien_Update", parameters, commandType: CommandType.StoredProcedure, transaction: transaction);
                            if (result != null)
                            {
                                try
                                {
                                    DynamicParameters parametersContent = new DynamicParameters();
                                    parametersContent.Add("@SuKienID", request.Entity.SuKienID);
