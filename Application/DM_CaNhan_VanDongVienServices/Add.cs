...
namespace Application.DM_CaNhan_VanDongVienServices
{
    public class Add
    {
        ...
        {
            public DM_CaNhanRequestHandleInfo Entity;
                        ...
                        try
                        {
                            ...
                            if (result != null)
                            {
                                ...
                                try
                                {
                                    ...
                                    parametersVDV.Add("@MaNgonNgu", request.Entity.MaNgonNgu);
                                    parametersVDV.Add("@TheChat", request.Entity.TheChat);
                                    parametersVDV.Add("@ThanhTich", request.Entity.ThanhTich);
                                    parametersVDV.Add("@GhiChu", request.Entity.GhiChu);
                                    var resultVDV = await connection.QueryFirstOrDefaultAsync<DM_CaNhan_VanDongVien_NoiDung>("spu_DM_CaNhan_VanDongVien_NoiDung_HandleInfo", parametersVDV, commandType: CommandType.StoredProcedure, transaction: transaction);
                                }
                                catch (Exception ex)
                                {
                                    transaction.Rollback();
                                ...
                                //Add relation CaNhan <=> MonTheThao
                                if (request.EntityTheThao != null && request.EntityTheThao.Count() > 0)
                                {
                                    foreach (var item in request.EntityTheThao)
                                    {
                                        try
                                        {

                                            DynamicParameters parametersTT = new DynamicParameters();
                                            parametersTT.Add("@CaNhanID", result.CaNhanID);
                                            parametersTT.Add("@VaiTroID", 2);
                                            parametersTT.Add("@MonTheThaoID", item.MonTheThaoID);
                                            parametersTT.Add("@MonTheThaoChinh", item.MonTheThaoChinh);
                                            var resultTT = await connection.QueryFirstOrDefaultAsync<DM_CaNhan_MonTheThao>("spu_DM_CaNhan_MonTheThao_Add", parametersTT, commandType: CommandType.StoredProcedure, transaction: transaction);
                                        }
                                        catch (Exception ex)
                                        {
                                            transaction.Rollback();
