                ...
                using (SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            var result = await connection.QueryFirstOrDefaultAsync<DM_CaNhan_NgheSi>("spu_DM_CaNhan_Add", parameters, commandType: CommandType.StoredProcedure, transaction: transaction);
                            ...
                            if (result != null)
                            {
                                ...
                                try
                                {
                                    ...
                                    parametersRelation.Add("@LinhVucID", 2);
                                    parametersRelation.Add("@NgheNghiepID", request.Entity.NgheNghiepID);
                                    var resultRelation = await connection.QueryFirstOrDefaultAsync<DM_CaNhan_NgheSi>("spu_DM_CaNhan_NgheSi_Add", parametersRelation, commandType: CommandType.StoredProcedure, transaction: transaction);
                                }
                                catch(Exception ex)
                                {
                                    transaction.Rollback();
