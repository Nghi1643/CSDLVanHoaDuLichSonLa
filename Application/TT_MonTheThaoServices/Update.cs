...
namespace Application.TT_MonTheThaoServices
{
    public class Update
    {
        ...
        {
            public TT_MonTheThao_UpdateRequest Entity;
        }
            ...
            public async Task<Result<TT_MonTheThao>> Handle(Command request, CancellationToken cancellationToken)
            {
                using (SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        parameters.Add("@MonTheTheoID", request.Entity.MonTheThaoID);
                        ...
                        parameters.Add("@NgayHieuChinh", DateTime.Now);
                        parameters.Add("@NguoiHieuChinh", userId);
                        parameters.Add("@TrangThai", request.Entity.TrangThai);
                        var result = await connection.QueryFirstOrDefaultAsync<TT_MonTheThao>("spu_TT_MonTheThao_Update", parameters, commandType: CommandType.StoredProcedure);
                        if (result != null)
                        {
                            ...
                            var resultContent = await connection.QueryFirstOrDefaultAsync<TT_MonTheThao_NoiDung>("spu_TT_MonTheThao_NoiDung_Update", parameterContent, commandType: CommandType.StoredProcedure);
                            if (resultContent == null)
                            {
                                return Result<TT_MonTheThao>.Failure("Cập nhật nội dung không thành công");
                            }

                            return Result<TT_MonTheThao>.Success(result);
                        }

                        return Result<TT_MonTheThao>.Failure("Cập nhật môn thể thao không thành công");
                    }
                    catch (Exception ex)
                    {
