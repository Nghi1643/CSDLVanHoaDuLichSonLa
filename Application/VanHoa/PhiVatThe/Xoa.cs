            ...
            public async Task<Result<bool>> Handle(Command request, CancellationToken cancellationToken)
            {
                using (var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            oldData = await _mediator.Send(new ChiTiet.Query { DiSanID = request.ID });
                        }
                        catch (Exception)
                        {
                            // Không trả lại lỗi
                        ...
                        parameters.Add("@DiSanID", request.ID);

                        var affectedRows = await connection.ExecuteAsync(
                            "spu_VH_DiSanPhiVatThe_Delete",
                            parameters,
                        ...
                        if (affectedRows > 0)
                        {
                            try
                            {
                                var log = await _mediator.Send(new Application.Audit.GhiLog.Command
                                {
                                    Data = new CSDL_Log()
                                    {
                                        ObjectID = oldData.Value.DiSanID.ToString(),
