            ...
            public async Task<Result<IEnumerable<DM_CaNhan_HuanLuyenVienViewModel>>> Handle(Query request, CancellationToken cancellationToken)
            {
                using(SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    try
                    {
                        ...
                        var result = await connection.QueryAsync<DM_CaNhan_HuanLuyenVienViewModel>("spu_DM_CaNhan_HuanLuyenVien_Gets", parameters, commandType: CommandType.StoredProcedure);
