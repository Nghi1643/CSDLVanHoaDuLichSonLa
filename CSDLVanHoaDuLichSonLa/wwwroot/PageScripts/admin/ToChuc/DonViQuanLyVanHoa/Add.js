...
function dataAdd() {
    ...
    if (checkEmptyBlank(id)) {
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Thêm mới đơn vị quản lý văn hóa thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/DonViQuanLyVanHoa/DonViQuanLyVanHoa`;
                    }, 1500);
                } else {
                    showNotification(0, "Thêm mới đơn vị quản lý văn hóa không thành công");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Chỉnh sửa đơn vị quản lý văn hóa thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/DonViQuanLyVanHoa/DonViQuanLyVanHoa`;
                    }, 1500);
                } else {
                    showNotification(0, "Chỉnh sửa đơn vị quản lý văn hóa không thành công");
                    console.log(data.error)
                }
            },
            error: function (err) {
...
function loadCoQuanData(id) {
    ...
    let data = JSON.stringify({
        loaiToChucID: 122, // đơn vị quản lý văn hóa
        toChucID: id
    })
    ...
    getDataWithApi('POST', '/api/ToChucApi/DanhSach', data).then(data => {
        ...
        if (data && data.isSuccess && data.value) {
            ...
            showNotification(0, 'Không tìm thấy thông tin đơn vị quản lý văn hóa');
        }
    })
}
