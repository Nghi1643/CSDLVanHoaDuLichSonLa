...
$(document).ready(function () {
        ...
        const tableApi = {
            ...
            data: function (d) {
                ...
                return JSON.stringify({
                    loaiToChucID: 100, // ID cho đơn vị nghệ thuật
        ...
        const tableDefs = [
            {
                ...
                render: function (data, type, row, meta) {
                    return `<div class="group-info">
                        <div class="info-main">
                            <a href="${baseUrl}/AdminTool/DonViNgheThuat/Details?id=${row.toChucID}" class="text-primary text-decoration-none">
                                ${row.tenToChuc || ''}
                            </a>
                        </div>
            ...
            {
                targets: 3, // Cột năm thành lập
                render: function (data, type, row, meta) {
                    return row.namThanhLap || '';
                }
            },
            {
                targets: 4, // Cột giải thưởng - danh hiệu
                render: function (data, type, row, meta) {
                    return `<div class="group-info">
                        <div class="info-main">
                            <a href="${baseUrl}/AdminTool/DonViNgheThuat/GiaiThuongDanhHieu?id=${row.toChucID}" class="text-primary text-decoration-none">
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    let html = "";
                    if (permitedEdit) {
                        html += `<a href="${baseUrl}/AdminTool/DonViNgheThuat/Add?id=${row.toChucID}" data-toggle="tooltip" title="Chỉnh sửa" class="text-yellow me-2">
    ...
    async function initLoaiHinhNgheThuat() {
        try {
            const res = await fetch(`${baseUrl}/api/DanhMucChungApi/DanhSach`, {
                ...
                body: JSON.stringify({
                    loaiDanhMucID: "24", // Loại hình nghệ thuật
                    trangThai: true
                })
            })
