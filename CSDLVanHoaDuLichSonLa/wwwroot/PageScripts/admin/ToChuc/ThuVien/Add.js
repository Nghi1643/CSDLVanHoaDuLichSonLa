...
function dataAdd() {
    ...
    if (checkEmptyBlank(id)) {
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Thêm mới thư viện thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/ThuVien/ThuVien`;
                    }, 1500);
                } else {
                    showNotification(0, "Thêm mới thư viện không thành công");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Chỉnh sửa thư viện thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/ThuVien/ThuVien`;
                    }, 1500);
                } else {
                    showNotification(0, "Chỉnh sửa thư viện không thành công");
                    console.log(data.error)
                }
            },
            error: function (err) {
...
function loadCoQuanData(id) {
    ...
    getDataWithApi('POST', '/api/ToChucApi/DanhSach', data).then(data => {
        ...
        if (data && data.isSuccess && data.value) {
            ...
            showNotification(0, 'Không tìm thấy thông tin thư viện');
        }
    })
}
