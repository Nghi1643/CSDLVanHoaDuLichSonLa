...
function dataAdd() {
    ...
    let dt = {
        "loaiToChucID": 96, // ID cho liên đoàn thể thao
    ...
    if (checkEmptyBlank(id)) {
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Thêm mới liên đoàn thể thao thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/LienDoan/LienDoan`;
                    }, 1500);
                } else {
                    showNotification(0, "Thêm mới liên đoàn thể thao không thành công");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Chỉnh sửa liên đoàn thể thao thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/LienDoan/LienDoan`;
                    }, 1500);
                } else {
                    showNotification(0, "Chỉnh sửa liên đoàn thể thao không thành công");
                    console.log(data.error)
                }
            },
            error: function (err) {
...
function loadCoQuanData(id) {
    ...
    let data = JSON.stringify({
        loaiToChucID: 96, // liên đoàn thể thao
        toChucID: id
    })
    ...
    getDataWithApi('POST', '/api/ToChucApi/DanhSach', data).then(data => {
        ...
        if (data && data.isSuccess && data.value) {
            ...
            showNotification(0, 'Không tìm thấy thông tin liên đoàn thể thao');
        }
    })
}
