...
function dataAdd() {
    ...
    let dt = {
        "loaiToChucID": 95, // ID cho bảo tàng
    ...
    if (checkEmptyBlank(id)) {
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Thêm mới bảo tàng thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/BaoTang/BaoTang`;
                    }, 1500);
                } else {
                    showNotification(0, "Thêm mới bảo tàng không thành công");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.isSuccess && data.value) {
                    showNotification(1, "Chỉnh sửa bảo tàng thành công");
                    console.log(data)
                    setTimeout(() => {
                        window.location.href = `${baseUrl}/AdminTool/BaoTang/BaoTang`;
                    }, 1500);
                } else {
                    showNotification(0, "Chỉnh sửa bảo tàng không thành công");
                    console.log(data.error)
                }
            },
            error: function (err) {
