...
1576
        ...
        $('#dataGrid tbody').on('click', '.edit-command-btn', async function () {
            ...
            $('#labelTenDanhMuc').html('Tên ngoại ngữ <span class="text-red">*</span>')
            $("#tenDanhMucAdd").attr("placeholder", "Nhập tên ngoại ngữ");
            $('#modalAddTitle').text(`Chỉnh sửa tên ngoại ngữ ${data.tenDanhMuc}`)
    ...
    $('#openModalAdd').on("click", function () {
        $('#labelTenDanhMuc').html('Tên ngoại ngữ <span class="text-red">*</span>')
        $("#tenDanhMucAdd").attr("placeholder", "Nhập tên ngoại ngữ");
        $('#modalAddTitle').text("Thêm mới tên ngoại ngữ")
        $('#maSoHide').removeClass('d-none')
