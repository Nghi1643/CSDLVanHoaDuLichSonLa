...
1576
        ...
        $('#dataGrid tbody').on('click', '.edit-command-btn', async function () {
            ...
            $('#labelTenDanhMuc').html('Tên loại chủ quản <span class="text-red">*</span>')
            $("#tenDanhMucAdd").attr("placeholder", "Nhập tên loại chủ quản");
            $('#modalAddTitle').text(`Chỉnh sửa tên loại chủ quản ${data.tenDanhMuc}`)
    ...
    $('#openModalAdd').on("click", function () {
        $('#labelTenDanhMuc').html('Tên loại chủ quản <span class="text-red">*</span>')
        $("#tenDanhMucAdd").attr("placeholder", "Nhập tên loại chủ quản");
        $('#modalAddTitle').text("Thêm mới tên loại chủ quản")
        $('#maSoHide').removeClass('d-none')
