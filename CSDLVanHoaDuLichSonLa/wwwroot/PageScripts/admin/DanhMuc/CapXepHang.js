...
1576
        ...
        $('#dataGrid tbody').on('click', '.edit-command-btn', async function () {
            ...
            $('#labelTenDanhMuc').html('Tên cấp xếp hạng <span class="text-red">*</span>')
            $("#tenDanhMucAdd").attr("placeholder", "Nhập tên cấp xếp hạng");
            $('#modalAddTitle').text(`Chỉnh sửa tên cấp xếp hạng ${data.tenDanhMuc}`)
    ...
    $('#openModalAdd').on("click", function () {
        $('#labelTenDanhMuc').html('Tên cấp xếp hạng <span class="text-red">*</span>')
        $("#tenDanhMucAdd").attr("placeholder", "Nhập tên cấp xếp hạng");
        $('#modalAddTitle').text("Thêm mới tên cấp xếp hạng")
        $('#maSoHide').removeClass('d-none')
