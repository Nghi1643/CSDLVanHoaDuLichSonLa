...
1576
        ...
        $('#dataGrid tbody').on('click', '.edit-command-btn', async function () {
            ...
            $('#labelTenDanhMuc').html('Tên loại hình sự kiện <span class="text-red">*</span>')
            $("#tenDanhMucAdd").attr("placeholder", "Nhập tên loại hình sự kiện");
            $('#modalAddTitle').text(`Chỉnh sửa tên loại hình sự kiện ${data.tenDanhMuc}`)
    ...
    $('#openModalAdd').on("click", function () {
        $('#labelTenDanhMuc').html('Tên loại hình sự kiện <span class="text-red">*</span>')
        $("#tenDanhMucAdd").attr("placeholder", "Nhập tên loại hình sự kiện");
        $('#modalAddTitle').text("Thêm mới tên loại hình sự kiện")
        $('#maSoHide').removeClass('d-none')
    ...
    function saveTempTranslation() {
        ...
        let index = translations.findIndex(t => t.maNgonNgu === maNgonNgu);

        // nếu cả tên và mô tả đều trống → xóa bản dịch
        if (!ten && !moTa) {
            if (index !== -1) {
        ...
        // nếu tồn tại thì cập nhật, không thì thêm mới
        if (index !== -1) {
            translations[index].ten = ten;
            translations[index].moTa = moTa;
        } else {
            translations.push({
                maNgonNgu: maNgonNgu,
                ten: ten,
    ...
    $("#formAdd").on("submit", async function (e) {
        ...
        translations = translations.filter(t => t.ten && t.ten.trim() !== "");
