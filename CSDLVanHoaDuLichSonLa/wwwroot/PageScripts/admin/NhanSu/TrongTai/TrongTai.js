...
function initTable() {
    const tableApi = {
        url: `${baseUrl}/api/DM_CaNhan_TrongTaiApi/Gets?MaNgonNgu=vi`,
        type: "GET",
    ...
    const tableDefs = [
        {
            ...
            render: function (data, type, row, meta) {
                return `<a href="${baseUrl}/AdminTool/NhanSu/Details_TrongTai?id=${row.caNhanID}" class="text-primary text-decoration-none"><div class="group-info have-image round-image">
        ...
        },
        {
            targets: 5, // Tần suất
            render: function (data, type, row, meta) {
                return `<a href="${baseUrl}/AdminTool/NhanSu/Details_TrongTai?id=${row.caNhanID}">
                            ${row.soLuongMon} môn
                        </a>`
            }
        },
        ...
        },
        {
            ...
            render: function (data, type, row, meta) {
                let html = "";
                if (permitedEdit) {
                    html += `<a href="${baseUrl}/AdminTool/NhanSu/Edit_TrongTai?id=${row.caNhanID}" data-toggle="tooltip" title="Chỉnh sửa" class="text-yellow me-2">
...
function dataDelete() {
    let id = $('#idDelete').val();
    $.ajax({
        url: `${baseUrl}/api/DM_CaNhan_TrongTaiApi/Delete?caNhanID=${id}`,
        type: 'DELETE',
