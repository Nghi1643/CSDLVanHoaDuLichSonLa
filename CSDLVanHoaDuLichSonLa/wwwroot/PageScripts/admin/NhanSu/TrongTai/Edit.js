...
$(document).ready(function () {
    ...
    //#region Xử lý lưu thông tin trọng tài
    setTimeout(() => {
        loadData(NhanSuId)
    }, 300);
    ...
    //#endregion Xử lý lưu thông tin trọng tài
})
...
//#region Function Thông tin trọng tài
function loadData(id) {
    getDataWithApi('GET', `/api/DM_CaNhan_TrongTaiApi/Get?CaNhanID=${id}&maNgonNgu=vi`).then(data => {
        if (data && data.isSuccess && data.value) {
            const item = data.value;
            console.log('Dữ liệu trọng tài:', item);
...
function dataEdit() {
    ...
    formData.append('RequestTrongTai', JSON.stringify(dtChung))
    formData.append("File", fileAnhDaiDien)

    $.ajax({
        type: 'PUT',
        url: `${baseUrl}/api/DM_CaNhan_TrongTaiApi/Update`,
        async: false,
        contentType: false,
        processData: false,
        data: formData,
        success: function (data) {
            if (data.isSuccess && data.value) {
                showNotification(1, "Chỉnh sửa trọng tài thành công");
                console.log(data)
                setTimeout(() => {
                    window.location.href = `${baseUrl}/AdminTool/NhanSu/TrongTai`;
                }, 1500);
            } else {
                showNotification(0, "Chỉnh sửa trọng tài không thành công");
    ...
    console.log('Dữ liệu trọng tài:', dtChung);
}

function loadTranslation(nhanSuId, maNgonNgu) {
    getDataWithApi('GET', `/api/DM_CaNhan_TrongTaiApi/Get?CaNhanID=${nhanSuId}&maNgonNgu=${maNgonNgu}`).then(data => {
...
//#endregion Function Thông tin trọng tài


//#region Function Xử lý bảng thể chất
function initTableTheThao(id) {
    const tableApi = {
        url: `${baseUrl}/api/DM_CaNhan_TrongTaiApi/GetListMon?CaNhanID=${id}&maNgonNgu=vi`,
        type: "GET",
    ...
    $('#tableTheThao').on('click', '.btnSave', function () {
        ...
        const apiUrl = isEdit
            ? `${baseUrl}/api/DM_CaNhan_TrongTaiApi/UpdateRelation`
            : `${baseUrl}/api/DM_CaNhan_TrongTaiApi/AddRelation`;
    ...
    function dataDelete() {
        ...
        $.ajax({
            url: `${baseUrl}/api/DM_CaNhan_TrongTaiApi/DeleteRelation?CaNhanID=${NhanSuId}&MonTheThaoID=${monTheThaoID}`,
            type: "DELETE",
            success: function (res) {
                if (res && res.isSuccess) {
