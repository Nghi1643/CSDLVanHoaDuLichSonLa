...
$(document).ready(function () {
    ...
    //#region Xử lý lưu thông tin vận động viên
    setTimeout(() => {
        loadData(NhanSuId)
    }, 300);
    ...
    //#endregion Xử lý lưu thông tin vận động viên
})
...
//#region Function Thông tin vận động viên
function loadData(id) {
    getDataWithApi('GET', `/api/DM_CaNhan_VanDongVienApi/Get?CaNhanID=${id}&maNgonNgu=vi`).then(data => {
        if (data && data.isSuccess && data.value) {
            const item = data.value;
            console.log('Dữ liệu vận động viên:', item);
...
function dataEdit() {
    ...
    formData.append('RequestVanDongVien', JSON.stringify(dtChung))
    formData.append("File", fileAnhDaiDien)

    $.ajax({
        type: 'PUT',
        url: `${baseUrl}/api/DM_CaNhan_VanDongVienApi/Update`,
        async: false,
        contentType: false,
        processData: false,
        data: formData,
        success: function (data) {
            if (data.isSuccess && data.value) {
                showNotification(1, "Chỉnh sửa vận động viên thành công");
                console.log(data)
                setTimeout(() => {
                    window.location.href = `${baseUrl}/AdminTool/NhanSu/VanDongVien`;
                }, 1500);
            } else {
                showNotification(0, "Chỉnh sửa vận động viên không thành công");
    ...
    console.log('Dữ liệu vận động viên:', dtChung);
}

function loadTranslation(nhanSuId, maNgonNgu) {
    getDataWithApi('GET', `/api/DM_CaNhan_VanDongVienApi/Get?CaNhanID=${nhanSuId}&maNgonNgu=${maNgonNgu}`).then(data => {
...
//#endregion Function Thông tin vận động viên


//#region Function Xử lý bảng thể chất
function initTableTheThao(id) {
    const tableApi = {
        url: `${baseUrl}/api/DM_CaNhan_VanDongVienApi/GetListMon?CaNhanID=${id}&maNgonNgu=vi`,
        type: "GET",
    ...
    $('#tableTheThao').on('click', '.btnSave', function () {
        ...
        const apiUrl = isEdit
            ? `${baseUrl}/api/DM_CaNhan_VanDongVienApi/UpdateRelation`
            : `${baseUrl}/api/DM_CaNhan_VanDongVienApi/AddRelation`;
    ...
    function dataDelete() {
        ...
        $.ajax({
            url: `${baseUrl}/api/DM_CaNhan_VanDongVienApi/DeleteRelation?CaNhanID=${NhanSuId}&MonTheThaoID=${monTheThaoID}`,
            type: "DELETE",
            success: function (res) {
                if (res && res.isSuccess) {
