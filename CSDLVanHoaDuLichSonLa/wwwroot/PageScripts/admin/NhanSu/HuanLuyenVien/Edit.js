...
$(document).ready(function () {
    ...
    //#region Xử lý lưu thông tin huấn luyện viên
    setTimeout(() => {
        loadData(NhanSuId)
    }, 300);
    ...
    //#endregion Xử lý lưu thông tin huấn luyện viên
})
...
//#region Function Thông tin huấn luyện viên
function loadData(id) {
    getDataWithApi('GET', `/api/DM_CaNhan_HuanLuyenVienApi/Get?CaNhanID=${id}&maNgonNgu=vi`).then(data => {
        if (data && data.isSuccess && data.value) {
            const item = data.value;
            console.log('Dữ liệu huấn luyện viên:', item);
...
function dataEdit() {
    ...
    formData.append('RequestHuanLuyenVien', JSON.stringify(dtChung))
    formData.append("File", fileAnhDaiDien)

    $.ajax({
        type: 'PUT',
        url: `${baseUrl}/api/DM_CaNhan_HuanLuyenVienApi/Update`,
        async: false,
        contentType: false,
        processData: false,
        data: formData,
        success: function (data) {
            if (data.isSuccess && data.value) {
                showNotification(1, "Chỉnh sửa huấn luyện viên thành công");
                console.log(data)
                setTimeout(() => {
                    window.location.href = `${baseUrl}/AdminTool/NhanSu/HuanLuyenVien`;
                }, 1500);
            } else {
                showNotification(0, "Chỉnh sửa huấn luyện viên không thành công");
    ...
    console.log('Dữ liệu huấn luyện viên:', dtChung);
}

function loadTranslation(nhanSuId, maNgonNgu) {
    getDataWithApi('GET', `/api/DM_CaNhan_HuanLuyenVienApi/Get?CaNhanID=${nhanSuId}&maNgonNgu=${maNgonNgu}`).then(data => {
...
//#endregion Function Thông tin huấn luyện viên


//#region Function Xử lý bảng thể chất
function initTableTheThao(id) {
    const tableApi = {
        url: `${baseUrl}/api/DM_CaNhan_HuanLuyenVienApi/GetListMon?CaNhanID=${id}&maNgonNgu=vi`,
        type: "GET",
    ...
    $('#tableTheThao').on('click', '.btnSave', function () {
        ...
        const apiUrl = isEdit
            ? `${baseUrl}/api/DM_CaNhan_HuanLuyenVienApi/UpdateRelation`
            : `${baseUrl}/api/DM_CaNhan_HuanLuyenVienApi/AddRelation`;
    ...
    function dataDelete() {
        ...
        $.ajax({
            url: `${baseUrl}/api/DM_CaNhan_HuanLuyenVienApi/DeleteRelation?CaNhanID=${NhanSuId}&MonTheThaoID=${monTheThaoID}`,
            type: "DELETE",
            success: function (res) {
                if (res && res.isSuccess) {
