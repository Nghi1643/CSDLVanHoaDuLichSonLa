...
function initTable() {
    const tableApi = {
        url: `${baseUrl}/api/DM_SuKienApi/Gets?MaNgonNgu=vi&LinhVucID=3`,
        type: "GET",
    ...
    const tableDefs = [
        {
            ...
            render: function (data, type, row, meta) {
                return `<div class="group-info">
                    <div class="info-main">
                        <a href="${baseUrl}/AdminTool/SuKienTheThao/Details?id=${row.suKienID}" class="text-primary text-decoration-none">
                            ${row.tenSuKien || ''}
                        </a>
                    </div>
        ...
        },
        {
            ...
            render: function (data, type, row, meta) {
                let html = "";
                if (permitedEdit) {
                    html += `<a href="${baseUrl}/AdminTool/SuKienTheThao/Edit?id=${row.suKienID}" data-toggle="tooltip" title="Chỉnh sửa" class="text-yellow me-2">
